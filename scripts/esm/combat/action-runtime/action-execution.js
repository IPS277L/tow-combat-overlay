...
export function createPanelActionExecutionService({
  ...
  async function runPanelAttackOnTarget(
    sourceToken,
    targetToken,
    attackItem,
    { autoRoll = true } = {}
    ...
    try {
      const testRef = await withTemporaryUserTargets(targetToken, () =>
        setupAbilityTestWithDamage(sourceActor, attackItem, {
          autoRoll
        })
      );
  ...
  async function runPanelUnarmedAttackOnTarget(
    sourceToken,
    targetToken,
    attackItem,
    { autoRoll = true } = {}
  ...
  async function runPanelAimAction(
    actor,
    sourceToken,
    { autoRoll = true, targetToken = null } = {}
  ) {
    if (!actor) return;
    const execute = async () => {
  ...
  async function runPanelHelpAction(
    actor,
    sourceToken,
    { autoRoll = true, targetToken = null } = {}
  ) {
