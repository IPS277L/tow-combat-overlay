export async function postFlowSeparatorCard(
  ...
  const attackerName =
    opposed?.attackerToken?.name ??
    towCombatOverlayLocalize('TOWCOMBATOVERLAY.Chat.Attacker', 'Attacker');
  const defenderName =
    opposed?.defenderToken?.name ??
    opposed?.defender?.alias ??
    towCombatOverlayLocalize('TOWCOMBATOVERLAY.Chat.Defender', 'Defender');
  ...
  const sourceStatusMarkup = (
    await Promise.all(
      toStatusItems(sourceStatusLabels).map((item) =>
        towCombatOverlayRenderTemplate(
  ...
  const targetStatusMarkup = (
    await Promise.all(
      toStatusItems(targetStatusLabels).map((item) =>
        towCombatOverlayRenderTemplate(
          'modules/tow-combat-overlay/templates/chat/rows/status-chip.hbs',
          item
        )
  ...
  const content = await towCombatOverlayRenderTemplate(
    'modules/tow-combat-overlay/templates/chat/flow-separator.hbs',
    {
  ...
  const chatData = {
    content,
    speaker: {
      alias: towCombatOverlayLocalize('TOWCOMBATOVERLAY.Chat.CombatFlowAlias', 'Combat Flow')
    }
  };
