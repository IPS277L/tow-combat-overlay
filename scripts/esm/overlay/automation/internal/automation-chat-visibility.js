export function hasRestrictedVisibility(message, { TOW_MODULE_ID, TOW_CHAT_VISIBILITY_FLAG } = {}) {
  ...
  if (typeof message.isContentVisible === 'boolean' && message.isContentVisible === false)
    return true;
...
export function resolveFlowVisibilitySourceMessage(
  ...
  if (hasRestrictedVisibility(opposedMessage, { TOW_MODULE_ID, TOW_CHAT_VISIBILITY_FLAG }))
    return opposedMessage;
  if (hasRestrictedVisibility(fallbackMessage, { TOW_MODULE_ID, TOW_CHAT_VISIBILITY_FLAG }))
  ...
  if (attackerMessageId) {
    ...
    if (hasRestrictedVisibility(attackerMessage, { TOW_MODULE_ID, TOW_CHAT_VISIBILITY_FLAG }))
  ...
  if (defenderMessageId) {
    ...
    if (hasRestrictedVisibility(defenderMessage, { TOW_MODULE_ID, TOW_CHAT_VISIBILITY_FLAG }))
