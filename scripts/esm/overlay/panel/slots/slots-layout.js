...
export function createPanelSlotsLayoutService({
  ...
  function buildTopChips(groups, actor) {
    const enableStatusRow = isTowCombatOverlayDisplaySettingEnabled(
      MODULE_SETTINGS.controlPanelEnableStatusRow,
      true
    );
    const enableAbilities =
      enableStatusRow &&
    ...
    const enableTemporaryEffects =
      ...
      isTowCombatOverlayDisplaySettingEnabled(
        MODULE_SETTINGS.controlPanelEnableTemporaryEffects,
        true
      );
    const abilities = enableAbilities
      ? Array.isArray(groups?.abilities)
        ? groups.abilities
        : []
      : [];
    const temporaryEffects = enableTemporaryEffects
      ? Array.isArray(groups?.temporaryEffects)
        ? groups.temporaryEffects
        : []
      : [];
  ...
  function updatePanelSlots(panelElement, token = null) {
    const enableGridButtons = isTowCombatOverlayDisplaySettingEnabled(
      MODULE_SETTINGS.controlPanelEnableGridButtons,
      true
    );
    const enableActionButtons =
      enableGridButtons &&
      isTowCombatOverlayDisplaySettingEnabled(
        MODULE_SETTINGS.controlPanelEnableActionButtons,
        true
      );
    const enableWeaponsButtons =
      enableGridButtons &&
      isTowCombatOverlayDisplaySettingEnabled(
        MODULE_SETTINGS.controlPanelEnableWeaponsButtons,
        true
      );
    const enableMagicButtons =
      enableGridButtons &&
        ...
        const normalizedItems = items
          ...
          .filter((item) => {
            ...
            if (key === 'attacks') {
              const itemId = String(item?.id ?? '')
                .trim()
                .toLowerCase();
              const unarmedId = String(panelUnarmedActionId ?? '')
                .trim()
                .toLowerCase();
          ...
          .map((item, index) => {
            const stableId = String(
              item?.id ?? item?.key ?? item?.name ?? `${key}-${index}`
            ).trim();
    ...
    const layoutKeys = Array.from(new Set([...savedLayoutKeys, ...defaultLayoutKeys]));
    ...
    const hasAccumulate = (Array.isArray(groupedItems.actions) ? groupedItems.actions : []).some(
      (item) =>
        String(item?.id ?? '')
          .trim()
          .toLowerCase() === 'accumulatepower'
