export function createControlPanelSlotMetaService({
  moduleId,
  ...
  function resolvePanelAttackAmmoMeta(item) {
    ...
    const attackSkill = String(item?.system?.attack?.skill ?? '')
      .trim()
      .toLowerCase();
    const weaponSkill = String(item?.system?.skill ?? '')
      .trim()
      .toLowerCase();
    const isRanged =
    ...
    const reloadTarget = Number.isFinite(reloadTargetRaw)
      ? Math.max(0, Math.trunc(reloadTargetRaw))
      : 0;
    return {
      isRanged: true,
      ammoMax,
  ...
  async function writePanelAttackAmmoState(item, { current, reloadProgress } = {}) {
    ...
    if (Number.isFinite(Number(current))) {
      updates[`flags.${moduleId}.panelAttackAmmo.current`] = Math.max(
        0,
        Math.trunc(Number(current))
      );
    }
    if (Number.isFinite(Number(reloadProgress))) {
      updates[`flags.${moduleId}.panelAttackAmmo.reloadProgress`] = Math.max(
        0,
        Math.trunc(Number(reloadProgress))
      );
    }
  ...
  function updatePanelSlotAttackAmmoVisualState(slotElement, item, groupKey, actor = null) {
    if (!(slotElement instanceof HTMLElement)) return;
    if (groupKey !== 'attacks' || !item || item.__empty === true)
      delete slotElement.dataset.attackReloadEmpty;
    else {
      const state = resolvePanelAttackAmmoState(item);
    ...
    if (groupKey !== 'magic' || !item || item.__empty === true)
      delete slotElement.dataset.magicInsufficientCv;
    else {
      ...
      const accumulated = Number.isFinite(rawAccumulated)
        ? Math.max(0, Math.trunc(rawAccumulated))
        : 0;
    ...
    if (
      ...
      const miscastsMax = Number.isFinite(miscastsMaxRaw)
        ...
        : 0;
      if (miscastsMax > 0 && miscasts >= miscastsMax)
        slotElement.dataset.accumulateMiscastReady = 'true';
  ...
  function updatePanelSlotPropertyBadge(slotElement, item, groupKey) {
    if (!(slotElement instanceof HTMLElement)) return;
    let propertyBadge = slotElement.querySelector(
