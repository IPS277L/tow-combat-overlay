export function createPanelSlotRenderService({
  ...
  function withOptionalDescription(hint, description) {
    ...
    if (normalizedHint && normalizedDescription)
      return `${normalizedHint}<br><br>${normalizedDescription}`;
  ...
  function ensureGroupSlotElements(panelElement, groupKey, slotCount) {
    ...
    let slotElements = Array.from(
      gridElement.querySelectorAll('.tow-combat-overlay-control-panel__slot')
    );
  ...
  function applyGroupGridLayout(panelElement, groupKey, slotCount) {
    ...
    const isStatusStripAbilities =
      gridElement.classList.contains('tow-combat-overlay-control-panel__status-abilities') ||
  ...
  function updateGroupSlots(panelElement, groupKey, groupItems = [], actor = null) {
    ...
    const includeClickBehaviorText =
      typeof showClickBehaviorText === 'function' ? !!showClickBehaviorText() : true;
    const groupElement =
      panelElement.querySelector(
    ...
    for (let index = 0; index < slotElements.length; index += 1) {
      ...
      const iconPlaceholder = slotElement.querySelector(
        '.tow-combat-overlay-control-panel__slot-icon'
      );
      ...
      if (!item || item.__empty === true) {
        ...
        const emptyOrderKey =
          String(item?.panelButtonKey ?? '').trim() ||
          toPanelButtonKey(emptyGroupKey, `empty-slot-${index}`);
        ...
        if (rarityEnabled) updatePanelSlotAttackRarity(slotElement, null, emptyGroupKey);
          ...
          const existingBadge = slotElement.querySelector(
            '.tow-combat-overlay-control-panel__slot-property'
          );
      ...
      slotElement.dataset.itemOrderKey = String(
        item?.panelButtonKey ?? toPanelButtonKey(resolvedGroupKey, item?.id)
      );
      ...
      if (resolvedGroupKey === 'attacks') {
        const attackHint = includeClickBehaviorText
          ? itemKey === panelUnarmedActionId
            ? localize(
                'TOWCOMBATOVERLAY.Tooltip.Panel.SlotHint.AttackUnarmed',
                '<em>Left click: pick target then auto-roll unarmed attack (Brawn vs Endurance) · Shift+click: self auto-roll · Alt+click: pick target then open default attack dialog · Alt+Shift+click: open default self-roll dialog</em>'
              )
            : localize(
                'TOWCOMBATOVERLAY.Tooltip.Panel.SlotHint.AttackWeapon',
                '<em>Left click: pick target then auto-roll attack · Shift+click: auto-roll self attack · Alt+click: pick target then open default Foundry attack dialog (no auto-roll) · Alt+Shift+click: open default Foundry self-roll dialog (no auto-roll) · Ctrl+click: instantly reload this weapon (ranged weapons with reload flow only)</em>'
              )
          : '';
        const attackAmmoState = resolvePanelAttackAmmoState(item);
        const reloadProgressHint =
          attackAmmoState.isRanged && attackAmmoState.usesReloadFlow && attackAmmoState.current <= 0
            ? `<br><br>${format(
                'TOWCOMBATOVERLAY.Tooltip.Panel.SlotHint.ReloadProgress',
                {
        ...
        const hasDistinctDescription =
          !!itemDescription &&
          (!specialPropertyMarkup || !itemDescription.includes(specialPropertyMarkup));
        ...
        let actionsHint = includeClickBehaviorText
          ? localize(
              'TOWCOMBATOVERLAY.Tooltip.Panel.SlotHint.ActionDefault',
              '<em>Left click: auto-flow action (auto-roll and auto-pick first) · Alt+click: default Foundry action dialogs</em>'
            )
          : '';
        if (itemKey === 'aim' && includeClickBehaviorText) {
          actionsHint = localize(
            'TOWCOMBATOVERLAY.Tooltip.Panel.SlotHint.ActionAim',
            '<em>Left click: pick target then auto-roll · Shift+click: self auto-roll · Alt+click: pick target then manual dialog · Alt+Shift+click: self manual dialog</em>'
          );
        } else if (itemKey === 'help' && includeClickBehaviorText) {
          actionsHint = localize(
            'TOWCOMBATOVERLAY.Tooltip.Panel.SlotHint.ActionHelp',
            '<em>Left click: pick target then auto-flow · Shift+click: self auto-flow · Alt+click: pick target then manual dialogs · Alt+Shift+click: self manual dialogs</em>'
          );
        } else if (itemKey === 'improvise' && includeClickBehaviorText) {
          actionsHint = localize(
            'TOWCOMBATOVERLAY.Tooltip.Panel.SlotHint.ActionImprovise',
            '<em>Left click: auto-pick first skill and auto-roll · Alt+click: manual skill selection and dialogs</em>'
          );
        } else if (itemKey === 'defence' && includeClickBehaviorText) {
          actionsHint = localize(
            'TOWCOMBATOVERLAY.Tooltip.Panel.SlotHint.ActionDefence',
            '<em>Left click: auto defence · Alt+click: manual defence selection dialog</em>'
          );
          ...
          actionsHint = includeClickBehaviorText
            ? miscastReady
              ? localize(
                  'TOWCOMBATOVERLAY.Tooltip.Panel.SlotHint.ActionAccumulatePowerReady',
                  '<em>Left click: roll on the miscast table · Ctrl+click: reset accumulated and potency values to 0</em>'
                )
              : localize(
                  'TOWCOMBATOVERLAY.Tooltip.Panel.SlotHint.ActionAccumulatePowerBuild',
                  '<em>Left click: auto-roll casting test with current roll modifiers to accumulate power · Alt+click: open default manual casting dialog · Ctrl+click: reset accumulated and potency values to 0</em>'
                )
            : '';
          const accumulatedRaw = Number(item?.system?.accumulatedPower ?? NaN);
          const accumulated = Number.isFinite(accumulatedRaw)
            ? Math.max(0, Math.trunc(accumulatedRaw))
            : 0;
        ...
        slotElement.dataset.tooltipDescription = withOptionalDescription(
          actionsHint,
          itemDescription
        );
        ...
        const recoverHint = includeClickBehaviorText
          ? itemKey === 'recover'
            ? localize(
                'TOWCOMBATOVERLAY.Tooltip.Panel.SlotHint.RecoverMain',
                '<em>Left click: run Recover with auto flow</em>'
              )
            : localize(
                'TOWCOMBATOVERLAY.Tooltip.Panel.SlotHint.Recover',
                '<em>Left click: run selected Recover action with auto flow · Alt+click: open manual dialogs for the selected Recover action</em>'
              )
          : '';
        slotElement.dataset.tooltipDescription = withOptionalDescription(
          recoverHint,
        ...
        const manoeuvreHint = includeClickBehaviorText
          ? localize(
              'TOWCOMBATOVERLAY.Tooltip.Panel.SlotHint.Manoeuvre',
              '<em>Left click: auto-roll manoeuvre checks (no dialogs) · Alt+click: default Foundry manoeuvre flow (with dialogs)</em>'
            )
          : '';
        slotElement.dataset.tooltipDescription = withOptionalDescription(
          manoeuvreHint,
        ...
        const infoHint = includeClickBehaviorText
          ? localize(
              'TOWCOMBATOVERLAY.Tooltip.Panel.SlotHint.TemporaryEffects',
              '<em>Right click: remove temporary effect.</em>'
            )
          : '';
        ...
        const abilitiesHint = includeClickBehaviorText
          ? localize(
              'TOWCOMBATOVERLAY.Tooltip.Panel.SlotHint.Abilities',
              '<em>Left click: show details.</em>'
            )
          : '';
        slotElement.dataset.tooltipDescription = withOptionalDescription(
          abilitiesHint,
        ...
        const magicHint = includeClickBehaviorText
          ? localize(
              'TOWCOMBATOVERLAY.Tooltip.Panel.SlotHint.Magic',
              '<em>Left click: cast and auto-process Apply buttons (with target pick when needed) · Shift+click: same cast flow, but using only currently selected targets (no target pick mode, no self fallback) · Alt+click: pick target then cast manually (no auto-apply)</em>'
            )
          : '';
        ...
        const magicStatsHint = [
          ...
          .filter(Boolean)
          .join('<br>');
        slotElement.dataset.tooltipDescription = joinTooltipBlocks(
          magicHint,
          magicStatsHint,
          itemDescription
        );
