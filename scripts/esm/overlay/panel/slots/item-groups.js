export function getPanelItemGroupsForActor(
  actor,
  {
  ...
  const abilityItems = toList(actor?.itemTypes?.ability).filter(
    (item) => !isPanelGeneratedUnarmedItem(item)
  );
  ...
  const attacks = abilityItems
    ...
    .concat(
      weaponItems.filter((item) => item?.system?.isEquipped || item?.system?.equipped?.value)
    );
  attacks.unshift({
    id: unarmedActionId,
    name: 'Unarmed Attack',
