...
export function getAllConditionEntries() {
  ...
  return Object.entries(conditions)
    .map(([id, data]) => {
      ...
      const img = String(
        data?.img ?? data?.icon ?? `/systems/whtow/assets/icons/conditions/${key}.svg`
      );
      return {
        id: key,
        img,
...
export function updatePanelWoundActionIndicator(
  panelElement,
  actor,
  { woundIconSrc, escapeHtml } = {}
) {
  ...
  let indicatorImage = indicatorElement.querySelector(
    '.tow-combat-overlay-control-panel__wound-action-indicator-image'
  );
  ...
  const indicatorData = getOverlayWoundIndicatorData(actor, {
    woundIconSrc,
...
export function getConditionTooltipData(conditionId) {
  ...
  const shortDescription = localizedDescription
    ...
    : '';
  const clickHint = localizeMaybe(
    'TOWCOMBATOVERLAY.Tooltip.Panel.ConditionToggleHint',
    '<em>Left click: toggle this condition.</em>'
  );
  return {
    title: String(
      name ||
        MODULE_TOOLTIPS?.conditionTitle ||
        localizeMaybe('TOWCOMBATOVERLAY.Label.Condition', 'Condition')
    ),
