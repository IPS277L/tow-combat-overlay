...
export function resolveTemporaryEffectDescription(effect) {
  ...
  const direct = normalizeDescriptionSource(
    effect?.description ?? effect?.system?.description ?? effect?.flags?.core?.description ?? ''
  );
  if (direct) return direct;

  const sourceItemFromGetter = effect?.sourceItem ?? null;
  const sourceItemUuid = String(effect?.system?.sourceData?.item ?? '').trim();
  const sourceItemFromUuid =
    !sourceItemFromGetter && sourceItemUuid && typeof fromUuidSync === 'function'
  ...
  if (sourceItem) {
    const fromSourceItem = normalizeDescriptionSource(
      sourceItem?.system?.description ?? sourceItem?.description ?? ''
    );
...
function normalizeDescriptionTextSource(descriptionSource) {
  ...
  if (raw && typeof raw === 'object') {
    raw = raw?.value ?? raw?.public ?? raw?.text ?? raw?.content ?? raw?.description ?? '';
  }
  ...
  temp.innerHTML = refTokenized.replace(/<br\s*\/?>/gi, '\n').replace(/<\/p>/gi, '</p>\n');
...
export function resolvePanelAttackSpecialPropertyText(item) {
  ...
  const isMeaningfulPropertyLine = (line) => {
    ...
    if (
      /^(close|short|medium|long|extreme)(\s*-\s*(close|short|medium|long|extreme))*$/i.test(value)
    )
      return false;
  ...
  const appendLines = (value) => {
    ...
    for (const line of normalized
      ...
      .filter((entry) => isMeaningfulPropertyLine(entry))) {
      lines.push(line);
    }
  };
  ...
  if (!lines.length) {
    const descriptionSource =
      item?.system?.description ?? item?.system?.summary ?? item?.description ?? '';
    const firstDescriptionLine =
      normalizeDescriptionTextSource(descriptionSource)
        .split('\n')
        .map((line) => line.trim())
        .find(Boolean) ?? '';
    const looksLikeExplicitProperty =
      /\b(on charge|ignore|ignores|success(?:es)?\s+to\s+reload|pierc|brutal|impale|bleed|stagger|prone|\+\d+\s*(?:damage|d\d+))\b/i.test(
        firstDescriptionLine
      );
    if (isMeaningfulPropertyLine(firstDescriptionLine) && looksLikeExplicitProperty)
      lines.push(firstDescriptionLine);
  }
