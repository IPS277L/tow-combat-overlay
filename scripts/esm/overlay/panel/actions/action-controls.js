export function createPanelActionControlsService({
  ...
  function updateActionControlsDisplay(panelElement, token = null) {
    ...
    const diceLabels = Array.from(
      panelElement.querySelectorAll("[data-action-label='diceModifier']")
    );
    const rollStateLabels = Array.from(
      panelElement.querySelectorAll("[data-action-label='rollState']")
    );
  ...
  function bindPanelActionControls(panelElement, { onUpdated } = {}) {
    ...
    for (const button of actionButtons) {
      ...
      if (control === 'diceModifier' && canShowButtonsTooltip())
        bindPanelTooltipEvent(button, getDiceModifierTooltipData);
      if (control === 'rollState' && canShowButtonsTooltip())
        bindPanelTooltipEvent(button, getRollStateTooltipData);

      button.addEventListener('click', async (event) => {
        event.preventDefault();
        if (control === 'diceModifier') {
          if (towCombatOverlayIsCtrlModifier(event)) {
            await withEditableSingleActor((actor) =>
              setTowCombatOverlayActorRollModifierState(actor, {
                ...getTowCombatOverlayActorRollModifierState(actor),
                diceModifier: 0
              })
            );
          ...
          await withEditableSingleActor((actor) =>
            adjustTowCombatOverlayActorRollModifierDice(actor, 1)
          );
          return;
        }
