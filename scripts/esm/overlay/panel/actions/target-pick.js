export function createPanelTargetPickService({
  ...
  function collectInteractiveTokenDisplayObjects() {
    ...
    const visit = (displayObject) => {
      ...
      const isInteractive =
        ...
        (typeof displayObject.eventMode === 'string' &&
          displayObject.eventMode !== 'none' &&
          displayObject.eventMode !== 'passive');
      if (isInteractive) results.push(displayObject);
      for (const child of Array.isArray(displayObject.children) ? displayObject.children : [])
        visit(child);
    };
  ...
  function applyPickCursorToTokenDisplayObjects(cursor = pickCursor, overrides = []) {
    const known = new Set(overrides.map((entry) => entry?.displayObject).filter(Boolean));
  ...
  function restoreTokenDisplayObjectCursors(overrides = []) {
    for (const entry of Array.isArray(overrides) ? overrides : []) {
      ...
      try {
        displayObject.cursor = entry.cursor;
      } catch (_error) {}
    }
  }
  ...
  function getWorldPointFromClientEvent(event) {
    ...
    const mapPositionToPoint =
      renderer?.events?.mapPositionToPoint ?? renderer?.plugins?.interaction?.mapPositionToPoint;
    if (typeof mapPositionToPoint !== 'function') return null;

    const localPoint = new PIXI.Point();
    mapPositionToPoint.call(
  ...
  function clearPickMode() {
    ...
    if (pending.windowPointerDownCapture)
      window.removeEventListener('pointerdown', pending.windowPointerDownCapture, true);
    if (pending.windowPointerMove)
      window.removeEventListener('pointermove', pending.windowPointerMove, true);
    if (pending.onEscape) window.removeEventListener('keydown', pending.onEscape, true);
    if (pending.onRightClickCancel)
      window.removeEventListener('contextmenu', pending.onRightClickCancel, true);
    if (pending.hoverTokenHookId != null) Hooks.off('hoverToken', pending.hoverTokenHookId);
    if (pending.panelElement instanceof HTMLElement)
      pending.panelElement.classList.remove('is-picking-attack');
    if (pending.slotElement instanceof HTMLElement)
      pending.slotElement.classList.remove('is-picking-attack');
  ...
  function startAttackPickMode(
    ...
    const onTargetAttack =
      typeof options?.onTargetAttack === 'function' ? options.onTargetAttack : null;
    const controlPanelState = getControlPanelState();
    ...
    const hoverTokenHookId = Hooks.on('hoverToken', () => {
      ...
      liveState.pendingAttackPick.displayObjectCursorOverrides =
        applyPickCursorToTokenDisplayObjects(
          pickCursor,
          liveState.pendingAttackPick.displayObjectCursorOverrides
        );
    });
    ...
    const resolveAttackPickTarget = async (targetToken, event = null) => {
      ...
      await runPanelAttackOnTarget(sourceToken, targetToken, attackItem, {
