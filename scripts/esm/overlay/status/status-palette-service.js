import { KEYS, STATUS_PALETTE_ICON_SIZE } from '../../runtime/overlay-constants.js';
import { clearStatusTooltip, hideStatusTooltip } from '../shared/shared.js';
import {
...
function getTokenLayoutPaletteVisibilitySettings() {
  ...
  const enableStatusRow = isTowCombatOverlayDisplaySettingEnabled(
    settings.tokenLayoutEnableStatusRow,
    true
  );
  return {
    enableStatusRow,
    ...
    enableStatusesTooltip:
      isTowCombatOverlayDisplaySettingEnabled(settings.tokenLayoutEnableTooltips, true) &&
    ...
    enableTemporaryEffectsTooltip:
      isTowCombatOverlayDisplaySettingEnabled(settings.tokenLayoutEnableTooltips, true) &&
      isTowCombatOverlayDisplaySettingEnabled(
        settings.tokenLayoutEnableTemporaryEffectsTooltip,
        true
      ),
...
export function setupStatusPalette(tokenObject) {
  ...
  const temporaryEffects = visibilitySettings.enableTemporaryEffects
    ? getTemporaryEffectEntries(actor)
    : [];
  const entries = [
    ...(visibilitySettings.enableStatuses ? allConditions : [])
      ...
      .map((entry) => {
        ...
        const displayName =
          String(conditionTooltipData?.name ?? entry?.id ?? '').trim() || 'Condition';
        return {
  ...
  const hiddenOverflowEntries =
    entries.length > STATUS_MAX_VISIBLE_CHIPS ? entries.slice(STATUS_MAX_VISIBLE_CHIPS) : [];
  const visibleEntries =
    entries.length > STATUS_MAX_VISIBLE_CHIPS
      ? [
          ...entries.slice(0, STATUS_MAX_VISIBLE_CHIPS),
          {
            ...
            tooltipData: visibilitySettings.enableOverflowTooltip
              ? {
                  ...
                  description:
                    buildOverflowTooltipListMarkup(hiddenOverflowEntries) ||
                    `+${hiddenOverflowEntries.length} more`
                }
              : { name: '', description: '' }
          }
        ]
  ...
  const expectedStateSignature = visibleEntries
    .map(
      (entry) =>
        `${String(entry?.key ?? '')}:${String(entry?.variant ?? '')}:${entry?.forceActive === true ? '1' : '0'}:${String(entry?.img ?? '')}`
    )
    .join('|');
  const { columns, iconSize, iconGap, chipsPerRow } = getStatusPaletteLayoutForToken(
    tokenObject,
    expectedCount
  );
  ...
  if (shouldRebuild) {
    ...
    for (let i = 0; i < visibleEntries.length; i++) {
      ...
      const sprite = isOverflow ? new PIXI.Container() : PIXI.Sprite.from(entry.img);
      ...
      if (isOverflow) {
        ...
        towCombatOverlayTuneTextForScale(
          overflowText,
          iconSize / Math.max(1, STATUS_PALETTE_ICON_SIZE)
        );
      ...
      towCombatOverlayBindTooltipHandlers(
        sprite,
        () => sprite?.[STATUS_TOOLTIP_DATA] ?? { name: '', description: '' },
        {
  ...
  const { totalWidth, totalHeight } = layoutStatusSpritesCentered(
    statusSprites,
    columns,
    iconSize,
    iconGap
  ...
  const fitScale = getStatusPaletteFitScale(
    tokenObject,
    totalWidth,
    iconSize,
    iconGap,
