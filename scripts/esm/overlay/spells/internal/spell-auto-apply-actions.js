...
export async function invokeAutoApplyActionsInMessage(
  messageId,
...
export async function waitAndInvokeAutoApplyActionsInMessage(
  ...
  for (let i = 0; i < total; i += 1) {
    ...
    const pendingBefore = actionsBefore.filter(
    ...
    for (const { action, dataset } of pendingBefore) {
      ...
      const syntheticTarget = { dataset: { ...dataset, action } };
      const done = await invokeChatMessageActionByName(currentMessage, action, syntheticTarget, {
