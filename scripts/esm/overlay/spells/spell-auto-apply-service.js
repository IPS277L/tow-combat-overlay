...
import { AUTO_APPLY_DIALOG_TIMEOUT_MS } from '../shared/auto-apply-action-constants.js';
import {
...
export function createPanelSpellAutoApplyService({
  ...
  function armAutoProcessSpellApplyButtons(
    actor,
    spell,
    {
    ...
    const processMessage = async (message) => {
      ...
      const performAutoApply = async () => {
        ...
        const restoreStaggerPrompt = automation.armDefaultStaggerChoiceWound(
          panelStaggerPatchDurationMs
        );
        const restoreSpellTestAuto = armAutoResolveSpellTriggeredTestDialogs(actor, {
          ...
          matches: (app) => {
            ...
            const appItemUuid = String(
              app?.context?.itemUuid ?? app?.context?.spellUuid ?? ''
            ).trim();
            ...
            if (Array.isArray(contextTargets)) {
              return contextTargets.some((target) => {
                ...
                return (
                  (sourceActorId && actorRef === sourceActorId) ||
                  (sourceActorUuid && actorRef === sourceActorUuid)
                );
              });
            }
            return false;
        ...
        try {
          const hasTargets =
            messageHasExplicitTargets(message) || (game?.user?.targets?.size ?? 0) > 0;
          const requiresTargetPick =
            !hasTargets &&
            !!sourceToken &&
            allowTargetPick &&
          ...
          startPanelAttackPickMode(
            ...
            originEvent,
            {
              onTargetAttack: async (targetToken) => {
                if (
                  targetToken?.actor &&
                  targetToken.actor.isOwner !== true &&
                  game?.user?.isGM !== true
                ) {
    ...
    const createHookId = Hooks.on('createChatMessage', (message) => {
      ...
      const actorUuid = String(actor?.uuid ?? '').trim();
      const sameActor = messageActorId === actorId || (!!actorUuid && testActorUuid === actorUuid);
      if (!sameActor) return;
      ...
      const itemUuid = String(
        message?.system?.context?.itemUuid ?? message?.system?.test?.context?.itemUuid ?? ''
      ).trim();
      const itemId = String(message?.system?.test?.item?.id ?? '').trim();
      const sameSpell =
        itemUuid === spellUuid || (itemId && itemId === String(spell?.id ?? '').trim());
      if (!sameSpell) return;
  ...
  async function runPanelCastSpecificSpellFromAccumulatedPower(
    actor,
    spell,
    {
    ...
    const matchesSpellCastingDialog = (app) => {
      ...
      const rollClass = String(app?.context?.rollClass ?? '')
        .trim()
        .toLowerCase();
