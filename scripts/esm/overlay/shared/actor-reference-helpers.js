function addUniqueActor(actors, actor) {
  ...
  if (
    actors.some(
      (entry) => entry === actor || String(entry?.uuid ?? '') === String(actor?.uuid ?? '')
    )
  )
    return;
  actors.push(actor);
...
export function collectMessageTargetActors(rawTargets) {
  ...
  if (Array.isArray(rawTargets)) {
    for (const entry of rawTargets) {
      addUniqueActor(
        actors,
        resolveActorFromReference(
          entry?.actor ?? entry?.actorId ?? entry?.token ?? entry?.tokenId ?? entry?.id ?? entry
        )
      );
    }
    return actors;
  ...
  if (rawTargets instanceof Set) {
    for (const entry of rawTargets)
      addUniqueActor(actors, resolveActorFromReference(entry?.id ?? entry));
    return actors;
  }
  ...
  if (rawTargets && typeof rawTargets === 'object') {
    for (const [key, value] of Object.entries(rawTargets)) {
      addUniqueActor(
        actors,
        resolveActorFromReference(
          value?.actor ?? value?.actorId ?? value?.token ?? value?.tokenId ?? value?.id ?? key
        )
      );
    }
  }
