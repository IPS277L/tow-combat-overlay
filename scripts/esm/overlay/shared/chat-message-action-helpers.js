export function parseDatasetFromTag(tagHtml) {
  ...
  const attrMatches = Array.from(
    String(tagHtml ?? '').matchAll(/\bdata-([a-z0-9_-]+)\s*=\s*["']([^"']*)["']/gi)
  );
  for (const match of attrMatches) {
    const rawKey = String(match?.[1] ?? '').trim();
    if (!rawKey) continue;
    const camelKey = rawKey.replace(/-([a-z0-9])/gi, (_m, char) =>
...
export function getMessageActionsByPrefix(
  message,
  { actionPrefix = '', priorityEntries = [] } = {}
) {
  const normalizedPrefix = String(actionPrefix ?? '')
    .trim()
    .toLowerCase();
