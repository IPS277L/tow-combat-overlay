...
import { writeSavedTopPanelPosition, writeSavedTopPanelTokenOrder } from './top-panel-state.js';
...
function bindTopPanelElementEvents(topPanelElement) {
  ...
  const canReorderTopPanelCards = () =>
    canTowCombatOverlayUserViewControl(settings.tokensPanelCardsDragDropMinimumRole, 'all');
  const isTopPanelTooltipsEnabled = () =>
    isTowCombatOverlayDisplaySettingEnabled(settings.tokensPanelEnableTooltips, true);
  ...
  const onPanelPointerMove = (event) => {
    ...
    applyTopPanelPosition(
      topPanelElement,
      panelDragData.startLeft + deltaX,
      panelDragData.startTop + deltaY
    );
  };
  ...
  const onPortraitHoverStateHide = (event) => {
    ...
    if (
      ...
      next.closest('.tow-combat-overlay-top-panel__portrait') === portraitElement
    )
      return;
  ...
  topPanelElement.addEventListener('pointerleave', () =>
    clearLinkedTopPanelHover(state, { panelElement: topPanelElement })
  );
  topPanelElement.addEventListener('pointercancel', () =>
    clearLinkedTopPanelHover(state, { panelElement: topPanelElement })
  );
  ...
  const onChipTooltipShow = (event) => {
    ...
    if (!title) return;
    const description =
      String(chipElement.dataset.tooltipDescription ?? '').trim() || 'No description.';
    showOverlayTooltip(title, description, { x: event.clientX, y: event.clientY }, null, {
      allowOutsideCanvas: true,
      clientCoordinates: true,
      theme: 'panel',
  ...
  const onChipTooltipHide = (event) => {
    ...
    if (
      next instanceof Element &&
      next.closest('.tow-combat-overlay-top-panel__chip') === chipElement
    )
      return;
    hideStatusTooltip();
  };
  ...
  const onControlTooltipHide = (event) => {
    ...
    if (
      next instanceof Element &&
      next.closest('.tow-combat-overlay-top-panel__control-button') === buttonElement
    )
      return;
    hideStatusTooltip();
  };
  ...
  topPanelElement.addEventListener('dragover', (event) => {
    ...
    const sourceId = String(
      liveState.draggedTokenId ?? event.dataTransfer?.getData('text/plain') ?? ''
    ).trim();
  ...
  topPanelElement.addEventListener('drop', (event) => {
    ...
    const sourceId = String(
      event.dataTransfer?.getData('text/plain') ?? liveState.draggedTokenId ?? ''
    ).trim();
    const targetId = String(targetPortrait.dataset.tokenId ?? '').trim();
    if (!sourceId || !targetId || sourceId === targetId) return;
    ...
    if (!sceneId) return;
    const currentTokenIds = getOrderedSceneTokens()
      .map((token) => String(token.id ?? '').trim())
      .filter(Boolean);
    const nextOrder = moveTokenRelativeToTarget(currentTokenIds, sourceId, targetId, {
      placeAfter
    });
...
export function towCombatOverlayRemoveTopPanel() {
  ...
  if (typeof state?.onPanelPointerMove === 'function')
    window.removeEventListener('pointermove', state.onPanelPointerMove);
