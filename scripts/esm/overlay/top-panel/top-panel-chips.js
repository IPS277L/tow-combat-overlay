...
function getTopPanelWoundStateChipData(actor, conditionId) {
  ...
  const description = String(
    stateData?.description ?? stateData?.effect?.document?.name ?? liveEffect?.name ?? ''
  ).trim();
...
export function getActiveConditionChips(actor) {
  ...
  for (const entry of entries) {
    ...
    const liveEffects = getActorEffectsByStatus(actor, id).filter(
      (effect) => effect && !effect.disabled && !effect.isSuppressed
    );
    const liveImage = String(liveEffects[0]?.img ?? liveEffects[0]?.icon ?? '').trim();
    chips.push({
      type: 'condition',
      key: `condition:${id}`,
      title: String(entry.label ?? id),
      description:
        woundStateChipData?.description || description || TOP_PANEL_CHIP_TOOLTIP_FALLBACK,
...
export function getTemporaryEffectChips(actor) {
  ...
  const conditionKeys = new Set(
    Object.keys(game?.oldworld?.config?.conditions ?? {}).map((key) =>
  ...
  return Array.from(actor?.effects?.contents ?? [])
    .filter((effect) => {
      ...
      const statuses = Array.from(effect.statuses ?? []).map((status) =>
    ...
    .map((effect) => ({
      ...
      title: String(
        effect?.name ?? localizeMaybe('TOWCOMBATOVERLAY.Label.Condition', 'Condition')
      ).trim(),
...
export function getWoundsAbilityChip(actor) {
  ...
  return {
    ...
    title: String(
      woundIndicatorData.title ?? localizeMaybe('TOWCOMBATOVERLAY.Tooltip.Wounds.Title', 'Wounds')
    ).trim(),
...
export function limitChipList(
  items,
  maxCount = TOP_PANEL_CHIP_MAX_PER_ROW,
...
function isChipTooltipEnabled(chip, tooltipConfig = {}) {
  const enabledByType =
    tooltipConfig &&
    typeof tooltipConfig.enabledByType === 'object' &&
    tooltipConfig.enabledByType !== null
      ? tooltipConfig.enabledByType
      : {};
  const chipType = String(chip?.type ?? '')
    .trim()
    .toLowerCase();
...
export function createTopPanelChipElement(chip, tooltipConfig = {}) {
  ...
  button.classList.add(
    'tow-combat-overlay-top-panel__chip',
    `is-${String(chip?.type ?? 'ability')}`
  );
