import { KEYS, PreciseTextClass, TOKEN_CONTROL_PAD } from '../../runtime/overlay-constants.js';
...
function overlayControlsBindTooltipHandlersRef(
  displayObject,
  getTooltipData,
  keyStore = null,
  options = null
) {
  return towCombatOverlayBindTooltipHandlers(
    displayObject,
    getTooltipData,
    keyStore,
    options ?? undefined
...
function getTokenLayoutNamePosition() {
  const { settings } = getTowCombatOverlayConstants();
  const raw = String(getTowCombatOverlayDisplaySetting(settings.tokenLayoutNamePosition, 'bottom'))
    .trim()
    .toLowerCase();
...
export function towCombatOverlayUpdateNameLabel(tokenObject) {
  ...
  if (!labelContainer[KEYS.nameLabelTooltipBinding]) {
    labelContainer[KEYS.nameLabelTooltipBinding] = overlayControlsBindTooltipHandlersRef(
      ...
      () => {
        const liveName =
          getPrimaryTokenName(tokenObject) ||
        ...
        return {
          ...
          description:
            ...
            description !== 'TOWCOMBATOVERLAY.Tooltip.Panel.SelectionTypeDescription'
              ? description
              : `Type: ${typeLabel}`
        };
      },
  ...
  if (namePosition === 'bottom') {
    ...
    nameText.position.set(
      0,
      Math.round(Number(tokenObject.h ?? 0) * inverseScale + tokenEdgePad - nameBounds.y)
    );
  } else {
    nameText.anchor.set(0.5, 1);
    ...
    nameText.position.set(
      0,
      Math.round(-tokenEdgePad - nameBottom + 2 * inverseScale * (edgePad / TOKEN_CONTROL_PAD))
    );
  }
  ...
  const tokenWidthWorld = Math.max(1, Number(tokenObject.w ?? 0));
  ...
  const postScaleBottom =
    labelContainer.y + (nameText.y + postScaleBounds.y + postScaleBounds.height) * finalLabelScale;
  if (namePosition === 'top') {
