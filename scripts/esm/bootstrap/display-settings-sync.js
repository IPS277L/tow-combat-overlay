...
export function syncTowCombatOverlayDisplaySettings(changedSettingKey = '') {
  ...
  const shouldRebuildControlPanel =
    normalizedChangedSettingKey === settings.controlPanelEnableStatuses ||
  ...
  if (normalizedChangedSettingKey === settings.controlPanelPositionMode) {
    const positionMode = String(
      getTowCombatOverlayDisplaySetting(settings.controlPanelPositionMode, 'free')
    ).trim();
  ...
  if (normalizedChangedSettingKey === settings.tokensPanelPositionMode) {
    const positionMode = String(
      getTowCombatOverlayDisplaySetting(settings.tokensPanelPositionMode, 'free')
    ).trim();
    if (positionMode === 'free') {
  ...
  const wantsEnabled =
    isTowCombatOverlayDisplaySettingEnabled(settings.enableOverlay, true) &&
    canTowCombatOverlayUserViewControl(settings.tokenLayoutMinimumRole, 'all');
  const wantsControlPanel =
    isTowCombatOverlayDisplaySettingEnabled(settings.enableControlPanel, true) &&
    canTowCombatOverlayUserViewControl(settings.controlPanelMinimumRole, 'all');
  const wantsTopPanel =
    isTowCombatOverlayDisplaySettingEnabled(settings.enableTopPanel, true) &&
  ...
  const isEnabled = typeof overlayApi.isEnabled === 'function' ? !!overlayApi.isEnabled() : false;
  ...
  if (wantsEnabled && typeof overlayApi.refreshAll === 'function') {
    void Promise.resolve(overlayApi.refreshAll()).catch((error) => {
      console.error(
        '[tow-combat-overlay] Failed to refresh token overlays after settings update.',
        error
      );
    });
  }
