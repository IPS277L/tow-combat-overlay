...
function resolveTowCombatOverlayHookBindings() {
  ...
  if (missing.length) {
    throw new Error(
      `${MODULE_LOG_PREFIX} Missing required overlay hook bindings: ${missing.join(', ')}`
    );
  }
  return bindings;
}
...
export function registerTowCombatOverlayHooks() {
  ...
  return {
    ...
    updateActor: Hooks.on('updateActor', (actor, changed) => {
      ...
      const hasRollModifierChange =
        !!moduleFlagChanges &&
        (MODULE_FLAGS.actorRollModifier in moduleFlagChanges || removeKey in moduleFlagChanges);
    ...
    createActiveEffect: Hooks.on('createActiveEffect', (effect) =>
      bindings.refreshActorOverlays(effect?.parent)
    ),
