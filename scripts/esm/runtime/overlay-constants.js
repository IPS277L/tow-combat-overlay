...
function resolvePreciseTextClass() {
  const preciseText =
    foundry?.canvas?.containers?.PreciseText ?? globalThis?.PreciseText ?? PIXI?.Text;
...
export const LAYOUT_BORDER_COLOR_NEUTRAL = 0xffe600;
export const LAYOUT_BORDER_COLOR_HOSTILE = 0xff0000;
export const LAYOUT_BORDER_COLOR_SECRET = 0xa2925a;
export const LAYOUT_BORDER_COLOR_CONTROLLED = 0x0088ff;
...
export function getLayoutBorderStyle(tokenObject) {
  ...
  const width = clampNumber(
    LAYOUT_BORDER_WIDTH * Math.pow(overlayScale, LAYOUT_BORDER_SCALE_EXP),
    0.68,
    2.1
  );
  const radius = clampNumber(
    LAYOUT_BORDER_RADIUS * Math.pow(overlayScale, LAYOUT_BORDER_RADIUS_SCALE_EXP),
    2.8,
    8.5
  );
...
export function getLayoutBorderColor(tokenObject) {
  ...
  if (Number.isFinite(secretDisposition) && disposition === secretDisposition)
    return LAYOUT_BORDER_COLOR_SECRET;
...
export function getStatusSpecialBgStyle(iconSize, baseFillAlpha) {
  ...
  const iconScale = clampNumber(
    sizeSafe / STATUS_PALETTE_ICON_SIZE,
    STATUS_SPECIAL_BG_ICON_SCALE_MIN,
    STATUS_SPECIAL_BG_ICON_SCALE_MAX
  );
  const pad = clampNumber(
    STATUS_PALETTE_SPECIAL_BG_PAD * Math.pow(iconScale, STATUS_SPECIAL_BG_RADIUS_SCALE_EXP),
    0.45,
    2.4
  );
  const outlineWidth = clampNumber(
    STATUS_PALETTE_SPECIAL_BG_OUTLINE_WIDTH *
      Math.pow(iconScale, STATUS_SPECIAL_BG_OUTLINE_SCALE_EXP),
    0.28,
    2.2
  );
  const radius = clampNumber(
...
export function getStatusPaletteBackdropStyle(iconSize) {
  ...
  return {
    ...
    radius: roundTo(
      clampNumber(STATUS_PALETTE_BACKDROP_RADIUS * Math.pow(iconScale, 0.98), 2.8, 10)
    ),
    fillAlpha: roundTo(
      clampNumber(STATUS_PALETTE_BACKDROP_FILL_ALPHA * (0.84 + iconScale * 0.16), 0.14, 0.34)
    )
  };
}
